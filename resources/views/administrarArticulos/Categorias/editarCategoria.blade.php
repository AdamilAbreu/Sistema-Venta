...
<body>
    ...
    <h1 class="text-white text-center mt-3">Lista de Categorias!</h1>
   <div  class="container d-flex gap-4">
    <div class="container cont-agregar-informacion mt-4 w-50">
        <form action="{{ url('categorias/' . $categoria ->id) }}" method="POST">
            @csrf
            @method('PUT')
            <h2 class="text-center text-white">Editar Categorias</h2>
            <div class="row mt-2">
                <label class="text-white  p-0"  for="nombre">Nombre</label>
                <input type="text" class="form-control" placeholder="Nombre" id="nombre" name="nombre" value="{{ $categoria ->nombre }}"  required>
            </div>
            <div class="row mt-3">
                <label class="text-white p-0" for="descripcion">Descripción</label>
                <input type="text" class="form-control" placeholder="Descripción" id="descripcion"  name="descripcion" value="{{ $categoria ->descripcion }}" required>
            </div>
    ...
    <div class="container mt-4 w-75">
        <table class="table table-striped table-active table-bordered custom-table text-center">
            ...
            <tbody>
                @foreach ($datosCategoria as $categorias)
                <tr>
                    <td>{{ $categorias->id }}</td>
