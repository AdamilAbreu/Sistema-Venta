...
<body>
   ...
   <div  class="container d-flex gap-4">
    <div class="container cont-agregar-informacion mt-4 w-50">
        <form action="{{ url('cliente/' . $usuario ->id) }}" method="POST">
            @csrf
            @method('PUT')
            <h2 class="text-center text-white">Editar Usuarios</h2>
            <div class="row mb-3">
                <div class="col-md-6 p-1 m-0">
                    <label class="text-white  p-0"  for="nombre">Nombre</label>
                    <input type="text" class="form-control" placeholder="Nombre" id="nombre" name="nombre" value="{{ $usuario->nombre }}" required>
                </div>
                <div class="col-md-6 p-1 m-0">
                    <label class="text-white p-0" for="role">Role</label>
                ...
                <div class="row mb-3">
                    <label class="text-white p-0" for="correo">Correo</label>
                     <input type="email" placeholder="Correo" class="form-control" name="correo" id="correo" value="{{ $usuario->correo }}" >
                </div>
                <div class="row mb-2">
                    <label class="text-white p-0" for="dirrecion">Dirección</label>
                     <input type="text" placeholder="Dirreción" class="form-control" name="dirrecion" id="dirrecion" value="{{ $usuario->dirrecion }}" >
                </div>
            <div class="d-flex align-item-center mt-4 gap-3 w-100">
                <button type="submit" class="btn btn-primary w-100">Actualizar</button>
                <a href="{{ route('VistaUsuario.indexUsuario') }}" class="btn btn-secondary w-100">Cancelar</a>
            </div>
        </form>
    </div>
